...
int8_t print_result(Image* img)
{
    ...
    if (TH_KPT < lowest_kpt_score)
    {
        ...
        for (i = 0; i < NUM_OUTPUT_C; i++)
        {
            arr.push_back(static_cast<int>(id_x[i]));
            arr.push_back(static_cast<int>(id_y[i]));
            arr.push_back(static_cast<int>(std::round(hrnet_preds[i][2] * 100)));
        }
        float data[63] = {};
        std::copy(arr.begin(), arr.end(), data);
        cv::Mat your_matrix = cv::Mat(1, 63, CV_32F, data);

        cv::Mat results={};
        dtree->predict(your_matrix,results);
        stream.str("");
        int class_num = results.at<float>(0);
        if (class_num == 8) return 0;
        stream<< "class: " <<label_file_map[class_num]<<".";
        str = stream.str();
        img->write_string_rgb(str, CAM_IMAGE_WIDTH * RESIZE_SCALE + TEXT_WIDTH_OFFSET, LINE_HEIGHT+LINE_HEIGHT_OFFSET+LINE_HEIGHT, CHAR_SCALE_SMALL, WHITE_DATA);
    }
    return 0;
}
