...
static std::string class_name = "car";
    ...
    if (!objects.empty()) {
            ...
            for (auto obj: objects) {
                ...
                for (int i = 1; i < centroidTracker->path_keeper[obj.first].size(); i++) {
                    ...
                    img.draw_line(centroidTracker->path_keeper[obj.first][i - 1].first, centroidTracker->path_keeper[obj.first][i - 1].second,centroidTracker->path_keeper[obj.first][i].first, centroidTracker->path_keeper[obj.first][i].second,WHITE_DATA);
                    x = centroidTracker->path_keeper[obj.first][i - 1].first;
                    y = centroidTracker->path_keeper[obj.first][i - 1].second;
                    k += 1;
                }
                // draw the centroid
