...
/*Tracker Variables*/
static std::string class_name = "person";
...
* Description   : Process CPU post-processing for Tiny YOLOv3
...
void draw_bounding_box(void)
{
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        cv::Mat bbox = (cv::Mat_<float>(1, 6) << det[i].bbox.x, det[i].bbox.y, det[i].bbox.w, det[i].bbox.h, det[i].prob, det[i].c);
...
void *R_Inf_Thread(void *threadid)
{
    ...
    while(1)
    {
        ...
        if (0 == inf_status)
        {
            ...
            /*CPU Post-Processing For Tiny YOLOv3*/
...
void *R_Capture_Thread(void *threadid)
{
    ...
    img_buffer0 = (unsigned char*) mmap(NULL, MIPI_CAM_IMAGE_WIDTH * MIPI_CAM_IMAGE_HEIGHT * MIPI_CAM_IMAGE_CHANNEL_YUY2 ,PROT_READ|PROT_WRITE, MAP_SHARED,  udmabuf_fd0, UDMABUF_INFIMAGE_OFFSET);
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if (!inference_start.load())
            {
                ...
                memcpy(img_buffer0, img_buffer, MIPI_CAM_IMAGE_WIDTH * MIPI_CAM_IMAGE_HEIGHT * MIPI_CAM_IMAGE_CHANNEL_YUY2);
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    if(argc>5)
    {
        ...
        line_x1=std::atoi(argv[2]);
        line_y1=std::atoi(argv[3]);
        line_x2=std::atoi(argv[4]);
        line_y2=std::atoi(argv[5]);

        if(argc>6)
        {
            DIRECTION = std::atoi(argv[6]);
        }
...
end_close_camera:
    ...
    if (0 != ret)
    {
        fprintf(stderr, "[ERROR] Failed to close MIPI Camera.\n");
        ret_main = -1;
    }
    delete capture;
    goto end_close_drpai;
