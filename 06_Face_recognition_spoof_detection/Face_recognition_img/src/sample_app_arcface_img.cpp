...
* Description   : Process CPU post-processing print the result on console.
...
int8_t print_result(float* floatarr)
{
    ...
    while(!fin.eof()){
        ...
        string name_buf = token;

        int i = 0;
        token = strtok(NULL, ",");
        while(token){
            embeddings[i] = stof(token, NULL);
            token = strtok(NULL, ",");
            i++;
        }
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    printf("Input : Windows Bitmap v3 | %s\n", input_img.c_str());
