...
***********************************************************************************************************************/
/***********************************************************************************************************************
* File Name    : sample_app_resnet18_tinyyolov3_cam.cpp
* Version      : 7.20
* Description  : RZ/V2L DRP-AI Sample Application for  ResNet-18 with TinyYOLOv3 MIPI Camera version
***********************************************************************************************************************/
...
static uint32_t resnet18_time[NUM_MAX_FACE];
static float resnet18_preds[2][NUM_MAX_FACE]; /* Pitch & Yaw */
static uint32_t id_x[2][NUM_MAX_FACE];
static uint32_t id_y[2][NUM_MAX_FACE];
static uint32_t id_x_local[2][NUM_MAX_FACE]; /*To be used only in Inference Threads*/
static uint32_t id_y_local[2][NUM_MAX_FACE]; /*To be used only in Inference Threads*/
...
/*****************************************
* Function Name : R_ResNet18_Coord_Convert
* Description   : Convert the post processing result into drawable coordinates
* Arguments     : n_pers = number of the detected face
* Return value  : -
******************************************/
static void R_ResNet18_Coord_Convert(uint8_t n_pers)
{
    /* Render skeleton on image and print their details */
    int32_t x1, x2;
    int32_t y1, y2;
    int32_t dx, dy;
    int32_t length = cropw[n_pers] / 2;

    /* Conversion from input image coordinates to display image coordinates. */
    /* Make sure the coordinates are not off the screen. */
    x1 = (int32_t)(cropx[n_pers] + (int32_t)cropw[n_pers]/2 + OUTPUT_ADJ_X);
    y1 = (int32_t)(cropy[n_pers] + (int32_t)croph[n_pers] * 1 /4 + OUTPUT_ADJ_Y);
    x1 = (x1 < 0) ? 0 : x1;
    x1 = (x1 > IMREAD_IMAGE_WIDTH - KEY_POINT_SIZE -1 ) ? IMREAD_IMAGE_WIDTH -KEY_POINT_SIZE -1 : x1;
    y1 = (y1 < 0) ? 0 : y1;
    y1 = (y1 > IMREAD_IMAGE_HEIGHT -KEY_POINT_SIZE -1) ? IMREAD_IMAGE_HEIGHT -KEY_POINT_SIZE -1 : y1;

    id_x_local[0][n_pers] = x1;
    id_y_local[0][n_pers] = y1;

    dx = (int32_t) -1 * length * cos(resnet18_preds[0][n_pers]) * sin(resnet18_preds[1][n_pers]);
    dy = (int32_t) -1 * length * sin(resnet18_preds[0][n_pers]);
    x2 = (int32_t) x1 + dx;
    y2 = (int32_t) y1 + dy;
    x2 = (x2 < 0) ? 0 : x2;
    x2 = (x2 > IMREAD_IMAGE_WIDTH - KEY_POINT_SIZE -1 ) ? IMREAD_IMAGE_WIDTH -KEY_POINT_SIZE -1 : x2;
    y2 = (y2 < 0) ? 0 : y2;
    y2 = (y2 > IMREAD_IMAGE_HEIGHT -KEY_POINT_SIZE -1) ? IMREAD_IMAGE_HEIGHT -KEY_POINT_SIZE -1 : y2;

    id_x_local[1][n_pers] = x2;
    id_y_local[1][n_pers] = y2;
    return;
}
...
static void draw_skeleton(void)
{
    ...
    if(face_count > 0)
    {
        ...
        {    
             ...
             /* Draw Gaze direction */
             img.draw_rect(id_x[0][i], id_y[0][i], KEY_POINT_SIZE, KEY_POINT_SIZE, WHITE_DATA);
             img.draw_line2(id_x[0][i], id_y[0][i], id_x[1][i], id_y[1][i], YELLOW_DATA);
             img.draw_rect(id_x[1][i], id_y[1][i], KEY_POINT_SIZE, KEY_POINT_SIZE, RED_DATA);
        }
    }
    return;
}
...
static int8_t print_result(Image* img)
{
    ...
    if (yolotime_prev != (uint32_t) yolo_time)
    {
        ...
        stream.str("");
        stream << "TinyYOLOv3 Time: " <<std::setw(3) << yolotime_prev << "msec";
        str1 = stream.str();
    }
    ...
    {
        stream.str("");
        stream << "ResNet-18(" << face_count << "face) Time: " << std::setw(3) << ai_time << "msec";
        str2 = stream.str();
        ...
        {
            stream.str("");
            stream << "Gaze Vector(Face" << face_count << ") : [" << resnet18_preds[0][i] << ", " << resnet18_preds[1][i] << "]";
            str2 = stream.str();
            img->write_string_rgb(str2, TEXT_WIDTH_OFFSET, LINE_HEIGHT*(4+i), CHAR_SCALE_SMALL, WHITE_DATA);
            stream.str("");
        }
    }
        ...
        while(1)
        {
            ...
            else
            {
                ...
                if (0 == inf_status)
                {
                    ...
                    /*If Face is detected run ResNet-18 for Pose Estimation three times*/
                    if(face_count_local > 0)
                    {
                        for(i = 0; i < face_count_local; i++)
                        {
                            ...
                            /*Change ResNet-18 Crop Parameters*/
                            ...
                            while(1)
                            {
                                ...
                                else
                                {
                                    ...
                                    resnet18_time[i] = resnet18_time[i] = (uint32_t)((timedifference_msec(start_time, inf_end_time)*TIME_COEF));
                                    ...
                                    if(inf_status == 0)
                                    {
                                        ...
                                        /*CPU Post Processing For ResNet-18, Random Forest & Display the Results*/
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    printf("Model :  ResNet-18 with TinyYOLOv3 | resnet18_cam, tinyyolov3_cam\n");
