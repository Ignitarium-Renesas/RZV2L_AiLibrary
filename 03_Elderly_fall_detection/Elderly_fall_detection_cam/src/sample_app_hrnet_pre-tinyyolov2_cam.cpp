...
* Return value  : -
******************************************/
static void people_counter(std::vector<detection>& det, std::vector<detection>& ppl, uint32_t box_count, uint32_t* ppl_count)
{
    uint32_t count = 0;
    ppl.clear();
...
* Description   : return the direction in which the given point lies based on the seperation line
* Arguments     : -
* Return value  : -
******************************************/
static int8_t get_direction(int x, int y)
{
    int d = ((x - line_x1) * (line_y2 - line_y1)) - ((y - line_y1) * (line_x2 - line_x1));
    return (d >= 0) ? 1: 0;
}
                        ...
                        for(i = 0; i < ppl_count_local; i++)
                        {
                            ...
                            while(1)
                            {
                                ...
                                else
                                {
                                    ...
                                    hrnet_time[i] = hrnet_time[i] = (uint32_t)((timedifference_msec(start_time, inf_end_time)*TIME_COEF));
