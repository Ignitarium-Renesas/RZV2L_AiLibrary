...
* Version      : 7.20
* Description  : RZ/V2L DRP-AI Sample Application for MMPose DeepPose with TinyYOLOv2 MIPI Camera version
***********************************************************************************************************************/
...
static int8_t print_result(Image* img)
{
    ...
    {
        stream.str("");
        stream << "DeepPose(" << face_count << "face) Time: " << std::setw(3) << ai_time << "msec";
        str2 = stream.str();
        img->write_string_rgb(str2, TEXT_WIDTH_OFFSET, LINE_HEIGHT*3, CHAR_SCALE_SMALL, WHITE_DATA);
        stream.str("");
        stream << "Gaze Direction: ";
        str0 = stream.str();
        img->write_string_rgb(str0, TEXT_WIDTH_OFFSET, LINE_HEIGHT*4, CHAR_SCALE_SMALL, WHITE_DATA);
        for(i=0; i<face_count; i++) 
        {
            stream.str("");
            stream << "Face" << i+1 << ": " << std::setw(3) << random_forest_preds[i];
            str0 = stream.str();
            img->write_string_rgb(str0, TEXT_WIDTH_OFFSET, LINE_HEIGHT*(5+i), CHAR_SCALE_SMALL, WHITE_DATA);
        }
    }
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    printf("Model : MMPose DeepPose with TinyYOLOv2 | deeppose_cam, tinyyolov2_cam\n");
